...
logger = logging.getLogger("property-system-api")
...
REPLY_CHECK_INTERVAL = 15  # Check for replies every 15 seconds

# Create FastAPI application
app = FastAPI(
    title="Property Matching System API",
    description="API for matching properties to clients and handling email communication",
    version="1.0.0"
)
...
class SystemStatusModel(BaseModel):
    ...
    timestamp: str
    running: bool
    last_property_match: Optional[str] = None
...
class SystemControlModel(BaseModel):
    ...
    run_property_matching: Optional[bool] = True
    check_email_replies: Optional[bool] = True


# Global variable to track if system is running
system_status = {
    "running": False,
    "last_property_match": None,
...
class PropertySystemAPI:
    """API wrapper for the Combined Property System"""

    def __init__(self):
        """Initialize the property system"""
        logger.info(f"Initializing Property System API at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        self.conn = self.connect_to_db()
        self.last_property_match_time = datetime.now() - timedelta(hours=1)
    ...
    def run_system_background(self, run_property_matching=True, check_email_replies=True):
        """Run the property system in background"""
        logger.info(f"\n{'=' * 80}\nStarting Property System in background thread\n{'=' * 80}")
        ...
        try:
            while not system_status["stop_requested"]:
                current_time = datetime.now()

                # Check if it's time to run property matching
                if run_property_matching:
                    ...
                    if time_since_last_match >= PROPERTY_MATCH_INTERVAL:
                        ...
                        system_status["last_property_match"] = current_time.strftime('%Y-%m-%d %H:%M:%S')

                # Check for email replies
                if check_email_replies:
                    logger.info(
                        f"\n{'=' * 60}\nChecking for email replies at {current_time.strftime('%Y-%m-%d %H:%M:%S')}")
                    self.check_email_replies()
                    system_status["last_email_check"] = current_time.strftime('%Y-%m-%d %H:%M:%S')

                # Sleep before next check
                logger.info(f"\nWaiting {REPLY_CHECK_INTERVAL} seconds before next check...")

                # Sleep in smaller increments to allow for quicker stopping
                for _ in range(REPLY_CHECK_INTERVAL // 5):
                    if system_status["stop_requested"]:
                        break
                    time.sleep(5)

        except Exception as e:
            error_msg = f"Error in system background thread: {str(e)}"
            ...
            logger.info("System background thread shutdown complete.")

    def stop_system(self):
        """Stop the background system"""
        global system_status
        logger.info("Stop request received for background system")
        system_status["stop_requested"] = True
        return {"status": "stopping", "message": "Stop signal sent to background system"}
    ...
    def get_system_status(self):
        ...
        return {
            ...
            "last_property_match": system_status["last_property_match"],
            "last_email_check": system_status["last_email_check"],
    ...
    def match_properties_for_clients(self):
        ...
        try:
            ...
            logger.info(f"Found {len(clients)} active clients for property matching")
    ...
    def send_match_email(self, client, matches):
        ...
        for i, match in enumerate(matches[:5]):  # Limit to top 5 matches
            ...
            contact_number = match.get('contact_number',
                                       'Not Available') if 'contact_number' in match else 'Not Available'
        ...
        try:
            ...
            logger.info(f"✅ Email successfully sent to {client['name']} ({client['email']})")
            return True
        except Exception as e:
    ...
    def save_matches_to_file(self, client, matches, file_path=None):
        ...
        try:
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(html)
            logger.info(f"Matches saved to file: {file_path}")
            return True
        except Exception as e:
            logger.error(f"Error saving matches to file: {str(e)}")
            return False
    ...
    def check_email_replies(self):
        ...
        try:
            ...
            if not email_ids:
                logger.info("No new replies found in the last 10 minutes")
                return

            logger.info(f"Found {len(email_ids)} emails since {date_string}. Processing...")
            recent_emails = 0
            ...
            for email_id in email_ids:
                ...
                logger.info(f"Time: {email_time.strftime('%Y-%m-%d %H:%M:%S')}")

                # Check if client exists in database
                client = self.get_client_by_email(from_address)

                if client:
                    logger.info(f"Matched to client: {client['name']} (ID: {client['id']})")

                    # Check if the reply indicates interest
                    if self.is_interested(body, subject):
                        logger.info(f"✅ Client shows interest! Scheduling meeting...")

                        # Schedule meeting and send follow-up
                        self.handle_interested_client(client, subject, body)
                    else:
                        logger.info(f"❌ No clear interest detected. Marking as reviewed.")
                else:
                    logger.info(f"⚠️ Email from {from_address} not matched to any client in database")
    ...
    def handle_interested_client(self, client, subject, body):
        ...
        if not matches:
            logger.info(f"No property matches found for client {client['name']}")
            return
        ...
        logger.info(f"Meeting scheduled for {client['name']} on {meeting_info['date']} at {meeting_info['time']}")
    ...
    def send_meeting_confirmation(self, client, matches, meeting_info):
        ...
        try:
            ...
            logger.info(f"✅ Meeting confirmation email sent to {client['name']} ({client['email']})")
            return True
        except Exception as e:
            logger.error(f"❌ Failed to send meeting confirmation email to {client['email']}: {str(e)}")
            return False


# Initialize the API service
property_system_api = PropertySystemAPI()


# API endpoints
@app.post("/api/system/control", response_model=SystemStatusModel)
async def control_system(control: SystemControlModel, background_tasks: BackgroundTasks):
    """
    Control the property system - start, stop, or check status.

    - **action**: "start", "stop", or "status"
    - **run_property_matching**: Whether to run property matching (default: true)
    - **check_email_replies**: Whether to check for email replies (default: true)
    """
    global system_status
    ...
    if control.action == "start":
        if system_status["running"]:
            return JSONResponse(
                ...
                content={"status": "error", "message": "System is already running", "running": True}
            )

        # Start the system in a background task
        background_tasks.add_task(
            property_system_api.run_system_background,
            run_property_matching=control.run_property_matching,
            check_email_replies=control.check_email_replies
        )
        ...
        if not system_status["running"]:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"status": "error", "message": "System is not running", "running": False}
            )

        # Stop the system
        property_system_api.stop_system()
        ...
        return property_system_api.get_system_status()

    else:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid action: {control.action}. Must be one of: start, stop, status"
        )
...
async def get_system_status():
    """
    Get the current status of the property system.
    """
    return property_system_api.get_system_status()
...
async def root():
    ...
    """
    return {
        "name": "Property Matching System API",
        "version": "1.0.0",
        "status": "online",
        "endpoints": [
            {"path": "/api/system/control", "method": "POST", "description": "Control the property system"},
            {"path": "/api/system/status", "method": "GET", "description": "Get system status"},
...
if __name__ == "__main__":
    print("=" * 80)
    print("Starting Property Matching System API")
    print("=" * 80)
