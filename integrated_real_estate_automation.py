...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("real_estate_automation.log"),
...
EMAIL_CHECK_INTERVAL = 600      # Check for email replies every 10 minutes

class RealEstateAutomation:
    """Combined class for property matching and email reply handling"""
    
    def __init__(self):
        """Initialize the automation system"""
        self.conn = self.connect_to_db()
        self.last_property_match_time = datetime.min
        self.last_email_check_time = datetime.min
        logger.info("Real Estate Automation System initialized")
    
    def connect_to_db(self, db_path=DB_PATH):
    ...
    def run_automation_cycle(self):
        ...
        if (current_time - self.last_property_match_time).total_seconds() >= PROPERTY_MATCH_INTERVAL:
            logger.info("Running property matching cycle...")
            self.run_property_matching()
            self.last_property_match_time = current_time
        
        # Check if it's time to check for email replies
        if (current_time - self.last_email_check_time).total_seconds() >= EMAIL_CHECK_INTERVAL:
            logger.info("Checking for email replies...")
            self.check_email_replies()
            self.last_email_check_time = current_time
    
    # ======== PROPERTY MATCHING FUNCTIONS ========
    
    def run_property_matching(self):
        """Run the property matching and notification process"""
        logger.info("Starting Property Matching and Notification Process")
        ...
        try:
            ...
            for client in clients:
                self.process_client_matching(client)
                
            logger.info("Property matching and notification process completed")
            
        except Exception as e:
            logger.error(f"Error in property matching: {e}", exc_info=True)
    
    def process_client_matching(self, client):
        """Process property matching for a single client"""
        ...
        if matches:
            ...
            if not email_sent:
                logger.warning(f"Saving matches to file for {client['name']}...")
    ...
    def save_matches_to_file(self, client, matches, file_path=None):
        ...
        try:
            ...
            logger.error(f"Error saving matches to file: {e}")
            return False
    ...
    def check_email_replies(self):
        ...
        logger.info(f"Checking for email replies from the last 10 minutes...")
        ...
        try:
            ...
            for email_id in email_ids:
                ...
                logger.info(f"Processing email from: {from_address}")
                ...
                if client:
                    ...
                    logger.warning(f"⚠️ Email from {from_address} not matched to any client in database")
            
            # Summary
            logger.info(f"Processed {recent_emails} emails from the last 10 minutes")
            
            # Close connection
            mail.logout()
            
        except Exception as e:
            logger.error(f"Error checking email replies: {e}", exc_info=True)
    ...
    def handle_interested_client(self, client, subject, body):
        ...
        if not matches:
            logger.warning(f"No property matches found for client {client['name']}")
            return
    ...
    def send_meeting_confirmation(self, client, matches, meeting_info):
        ...
        try:
            ...
            logger.error(f"❌ Failed to send meeting confirmation email to {client['email']}: {e}")
            return False
...
def main():
    """Main function to run the entire automation system"""
    logger.info("=" * 80)
    logger.info("Starting Integrated Real Estate Automation System")
    logger.info("=" * 80)
    logger.info(f"Current time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"Database path: {DB_PATH}")
    logger.info(f"Property matching interval: {PROPERTY_MATCH_INTERVAL//60} minutes")
    logger.info(f"Email checking interval: {EMAIL_CHECK_INTERVAL//60} minutes")
    logger.info("=" * 80)
    ...
    try:
        # Run property matching immediately on startup
        logger.info("Running initial property matching...")
        automation.run_property_matching()
        
        # Run the main loop
        logger.info("Starting main automation loop...")
        while True:
            try:
                # Run a cycle of the automation system
                automation.run_automation_cycle()
                
                # Sleep for a bit to prevent high CPU usage
                time.sleep(60)  # Check every minute if it's time to run tasks
        ...
        # Clean up
        automation.close()
        logger.info("Integrated Real Estate Automation System shutdown complete")
